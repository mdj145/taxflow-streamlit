...
st.write(
    'העלה קובץ CSV או Excel עם העמודות: date, description, amount. '
    'האפליקציה תחשב תזרים חודשי, חזוי 30 יום, ואומדן מס לפי קובץ כללים.'
)
...
def build_pdf(monthly: dict, forecast: float, est_tax: float) -> BytesIO:
    ...
    y -= 24
    c.setFont('Helvetica-Bold', 12)
    c.drawString(40, y, 'אומדן מס שנתי (לפי כללים):')
    y -= 18
    c.setFont('Helvetica', 11)
...
try:
    ...
    st.error("קובץ הכללים 'tax_rules_il_2025.yaml' חסר או פגום. ודא שהוא בשורש הפרויקט.")
    TAX_RULES = None
...
with st.expander('איך מעדכנים את כללי המס (בלי קוד)'):
    st.markdown(
        '1. פתחו את הקובץ **tax_rules_il_2025.yaml** בריפו של GitHub.\n'
        '2. עדכנו שם את המדרגות (up_to, rate), ואת surtax_threshold/surtax_rate אם יש שינוי.\n'
        '3. Commit. ב-Streamlit לחצו Restart – והחישוב יתעדכן אוטומטית.'
    )
